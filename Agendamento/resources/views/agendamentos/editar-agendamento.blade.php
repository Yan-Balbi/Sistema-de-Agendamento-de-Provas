...
<div class="bg-gray-100 p-6">
    ...
    <div class="max-w-2xl mx-auto bg-white shadow-md rounded-lg p-6 mt-10">
        ...
        <form method="POST" action="{{ route('agendamentos.update', $agendamentos->id) }}">
            ...
            <div placeholder="Selecione uma data">
                ...
                <input type="date" id="data" name="data" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-auto ps-10 p-2.5">
            </div>
            <br>
    ...
    <script>
        $(document).ready(function () {
            ...
            let horarioSelecionadoId = "{{ $intervaloDeHora->id }}";
            // Preencher ComboBox de Horarios
            $.get('/agendamento/index-horarios', function (data) {
                horariosData = data;
                data.forEach(horario => {
                    let option = new Option(horario.hora_inicial+' - '+horario.hora_final, horario.id);
                    $('#horario').append(option);
                    if(horario.id == horarioSelecionadoId){
                        $(option).prop('selected',true);
                    }
                });
                 // Armazenar os dados de horários na variável global
