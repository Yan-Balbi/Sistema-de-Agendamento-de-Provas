...
<div class="bg-gray-100 p-6">
    ...
    <div class="max-w-2xl mx-auto bg-white shadow-md rounded-lg p-6 mt-10">
        ...
        <form method="POST" action="{{ route('professor.update', $professor->id) }}" class="flex flex-col flex space-y-5">
            @csrf
            @method('PUT')
            <div>
                <input type="text" name="namee" placeholder="Digite o nome da sala" value="{{ $professor->nome }}" required class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm transition duration-300 ease-in-out focus:outline-none">
            </div>
