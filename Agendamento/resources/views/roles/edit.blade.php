...
@section('content')
<head>
    <link rel="stylesheet" href="{{ asset('CSS/estilo-editar-role.css') }}" />
    <script src="https://cdn.tailwindcss.com"></script>
</head>

<div class="bg-gray-100 p-6">
    <div class="max-w-2xl mx-auto bg-white shadow-md rounded-lg p-6 mt-10">
        <h1 class="text-xl font-semibold text-gray-800 mb-4">Editar Função</h1>
        ...
        <form action="{{ route('roles.update', $role->id) }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <input type="text" name="name" id="name" class="w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 transition" value="{{ $role->name }}" required>
            </div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 gap-2">
                    @foreach($permission as $value)
                        <label class="flex items-center space-x-2">
                            <input type="checkbox" name="permission[{{ $value->id }}]" value="{{ $value->id }}" class="form-checkbox text-blue-600" {{ in_array($value->id, $rolePermissions) ? 'checked' : '' }}>
