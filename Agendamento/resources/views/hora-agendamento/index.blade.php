...
<div class="bg-gray-100 p-6">
    <div class="max-w-5xl mx-auto bg-white shadow-md rounded-lg p-6 mt-5">
        ...
        <a href="{{ route('hora-agendamento.create') }}" class="bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600 transition">Adicionar Horário</a>
                ...
                @foreach ($horarios as $horario)
                    <tr class="bg-white border-b hover:bg-gray-100 transition">
                        ...
                        <td class="p-3 flex space-x-2">
                            <a href="{{ route('hora-agendamento.edit', $horario->id) }}" class="bg-yellow-500 text-white px-3 py-1 rounded hover:bg-yellow-600 transition text-sm">Editar</a>
                            <form action="{{ route('hora-agendamento.destroy', $horario->id) }}" method="POST" onsubmit="return confirm('Tem certeza que deseja excluir?')">
                                @csrf
                                @method('DELETE')
